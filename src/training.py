...
def eval_one_epoch(model, med, data_eval, voc_size, ddi_adj, DDI_store_path ):
    smm_record, ja, prauc, avg_p, avg_r, avg_f1 = [[] for _ in range(6)]
    med_cnt, visit_cnt = 0, 0
    our_test_med_frequency = np.zeros(voc_size[2])
    ...
    for step, input_seq in enumerate(data_eval):
        ...
        for adm_idx, adm in enumerate(input_seq):
            ...
            output, _ = model(
                med, input_seq[:adm_idx + 1], ddi_adj)
            ...
            our_test_med_frequency = our_test_med_frequency + y_pred_tmp
    ...
    with open('our_test_med_frequency.pkl', 'wb') as picklefile:
        pickle.dump(our_test_med_frequency, picklefile)

    ddi_rate = ddi_rate_score(smm_record, path=DDI_store_path)
...
def Test(model, model_path, device, data_test, med, voc_size, ddi_adj, dataset_path):
    with open(model_path, 'rb') as Fin:
        model.load_state_dict(torch.load(Fin, map_location=device))
    model = model.to(device).eval()
    DDI_store_path = '../data/' + dataset_path + '/ddi_A_final.pkl'
    ...
    for _ in range(10):
        ...
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med = \
            eval_one_epoch(model, med, test_sample, voc_size, ddi_adj, DDI_store_path)
...
def Train(
    model, device, data_train, data_eval, data_test, med, voc_size, ddi_adj,
    optimizer, log_dir, coef, target_ddi, EPOCH, dataset_path,
):
    ...
    for epoch in range(EPOCH):
        ...
        for step, input_seq in enumerate(data_train):
            for adm_idx, adm in enumerate(input_seq):
                ...
                result, loss_ddi = model(med, input_seq[:adm_idx + 1], ddi_adj)
            ...
            llprint('\rtraining step: {} / {} loss:{:.4f} len:{}'.format(step, len(data_train), loss, len(y_label)))
        ...
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med = \
            eval_one_epoch(model, med, data_eval, voc_size, drug_data, DDI_store_path)
