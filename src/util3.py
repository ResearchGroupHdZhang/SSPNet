...
def buildPrjSmiles(molecule, med_voc, device="cpu:0"):
    ...
    for index, ndc in med_voc.items():
        ...
        for smiles in smilesList:
           ...
           #  mol = Recap.RecapDecompose(smiles)
...
########################set loss#########################
def hamming_loss(predict_set, target_set, voc_size):
    predict_label = torch.zeros(voc_size[2])
    target_label = torch.zeros(voc_size[2])
    predict_label[predict_set] = 1
    target_label[target_set] = 1
    predict_label = predict_label.cpu().numpy()
    target_label = target_label.cpu().numpy()
    dec = np.sum(predict_label != target_label)
    loss = 1. * dec / (sum(predict_label) + sum(target_label))
    return loss
